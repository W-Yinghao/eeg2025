...
################################################################################
# Exp 6B — Gentle Partial FT Baseline: P3 (stage1=2 epochs)
#
# Staged partial fine-tuning:
#   Stage 1: top1 unfrozen, 2 epochs, lr_bb=1e-5, lr_head=1e-3, warmup=1
...
#
# Usage:
#   sbatch run_exp6b_gpartial_p3e2_baseline_jeanzay.sh [MODEL] [DATASET] [SEED]
################################################################################
...
MODE="baseline"
EXP_TAG="exp6b_gpartial_p3e2_baseline"

SAVE_DIR="${PROJECT_DIR}/checkpoints_selector/${EXP_TAG}"
LOG_DIR="${PROJECT_DIR}/deb_log/${EXP_TAG}"
mkdir -p "$SAVE_DIR" "$LOG_DIR"

TAG="Exp6B_P3e2_${MODE}_${MODEL}_${DATASET}_staged_s${SEED}_$(date +%Y%m%d_%H%M%S)"

echo "=============================================="
echo "  Exp 6B P3e2: Staged Partial FT Baseline (stage1=2ep)"
echo "  ${MODE} | ${MODEL} | ${DATASET} | seed=${SEED}"
echo "  Stage1: top1, 2ep, lr_bb=1e-5, lr_head=1e-3"
...
if [ $EXIT_CODE -eq 124 ]; then
    ...
    sbatch -o "${LOG_DIR}/%j_p3e2_s${SEED}.out" \
           -e "${LOG_DIR}/%j_p3e2_s${SEED}.err" \
           --job-name="E6B_p3e2_s${SEED}" \
