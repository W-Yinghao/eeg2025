...
################################################################################
# Exp 6B — Gentle Partial FT Baseline: P2
#
# Config:
#   regime=top1_gentle (last 1 block unfrozen, patch_embed frozen)
#   lr_head=1e-3, lr_backbone=1e-6 (1000x ratio)
...
#
# Usage:
#   sbatch run_exp6b_gpartial_p2_baseline_jeanzay.sh [MODEL] [DATASET] [SEED]
################################################################################
...
MODE="baseline"
REGIME="top1_gentle"
EXP_TAG="exp6b_gpartial_p2_baseline"

SAVE_DIR="${PROJECT_DIR}/checkpoints_selector/${EXP_TAG}"
LOG_DIR="${PROJECT_DIR}/deb_log/${EXP_TAG}"
mkdir -p "$SAVE_DIR" "$LOG_DIR"

TAG="Exp6B_P2_${MODE}_${MODEL}_${DATASET}_${REGIME}_s${SEED}_$(date +%Y%m%d_%H%M%S)"

echo "=============================================="
echo "  Exp 6B P2: Gentle Partial FT Baseline"
echo "  ${MODE} | ${MODEL} | ${DATASET} | regime=${REGIME} | seed=${SEED}"
echo "  lr_head=1e-3  lr_backbone=1e-6  warmup=3  epochs=12"
echo "  Node: $(hostname)"
...
if [ $EXIT_CODE -eq 124 ]; then
    ...
    sbatch -o "${LOG_DIR}/%j_p2_s${SEED}.out" \
           -e "${LOG_DIR}/%j_p2_s${SEED}.err" \
           --job-name="E6B_p2_s${SEED}" \
