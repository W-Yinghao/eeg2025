...
################################################################################
# Experiment 7C-main — Frozen Selector + Joint Sparse + Consistency (Jean Zay)
#
# Standard joint regularization: sparse on BOTH gates + consistency.
# This is the primary 7C version (not branch-aware ablation).
#
# Loss: L = L_ce + 1e-3 * L_sparse(temporal+frequency) + 3e-3 * L_consistency
#
# Sparse: uniform L1 on both temporal and frequency gates (Exp7A style)
# Consistency: L2 on augmented gate maps (Exp7B style, λ=3e-3)
#
# Usage:
#   sbatch run_exp7c_main_selector_jeanzay.sh [MODEL] [DATASET] [SEED]
################################################################################
...
SCRIPT_PATH="${PROJECT_DIR}/experiments/deb/scripts/run_exp7c_main_selector_jeanzay.sh"
mkdir -p "$SAVE_DIR" "$LOG_DIR"

TAG="Exp7C_main_${MODE}_${MODEL}_${DATASET}_s${SEED}_$(date +%Y%m%d_%H%M%S)"

echo "=============================================="
echo "  Exp 7C-main: Joint Sparse + Consistency"
...
if [ "$EXIT_CODE" -eq 124 ]; then
    ...
    NEW_JOB=$(sbatch \
        -o "${LOG_DIR}/%j_selector_frozen_7c_main_s${SEED}.out" \
