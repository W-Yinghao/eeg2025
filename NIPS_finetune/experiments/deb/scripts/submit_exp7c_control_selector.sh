#!/bin/bash
################################################################################
# Submit Exp 7C-control — Light Sparse + Consistency (gpu-gw)
#
# 3 seeds x 1 config = 3 jobs
#
# Usage:
#   bash experiments/deb/scripts/submit_exp7c_control_selector.sh [MODEL] [DATASET]
################################################################################
...
echo "=============================================="
echo "  Submitting Exp 7C-control: Light Sparse + Consistency (gpu-gw)"
...
for SEED in "${SEEDS[@]}"; do
    ...
    sbatch -o "${LOG_DIR}/%j_selector_frozen_7c_ctrl_s${SEED}.out" \
           ...
           "${SCRIPT_DIR}/run_exp7c_control_selector.sh" \
           "$MODEL" "$DATASET" "$SEED"
    COUNT=$((COUNT + 1))
done
