...
class MITrainer:
    ...
    def __init__(
        ...
        # Optimizer: separate param groups for head vs adapter params
        adapter_params = []
        head_params = []
        ...
        param_groups = [{'params': head_params, 'weight_decay': params.weight_decay}]
        if adapter_params:
            param_groups.append({
    ...
    def train(self) -> Tuple[float, float, float]:
        best_val_acc = -1
        best_epoch = 0
        patience = getattr(self.params, 'patience', 0)
        no_improve_count = 0
...
def parse_args():
    ...
    # Backbone model
    p.add_argument('--model', type=str, default='codebrain',
                   choices=['codebrain', 'cbramod', 'femba', 'luna'], help='backbone model type')
    # CodeBrain-specific
    p.add_argument('--n_layer', type=int, default=8, help='SSSM residual layers (CodeBrain)')
    p.add_argument('--codebook_size_t', type=int, default=4096)
    p.add_argument('--codebook_size_f', type=int, default=4096)
    # CBraMod-specific
    p.add_argument('--n_layer_cbramod', type=int, default=12, help='transformer layers (CBraMod)')
    p.add_argument('--nhead', type=int, default=8, help='attention heads (CBraMod)')
    p.add_argument('--dim_feedforward', type=int, default=800, help='FFN dim (CBraMod)')
    # Inter-layer adapters (CBraMod only)
    p.add_argument('--use_layer_adapters', action='store_true',
    ...
    # LUNA-specific
    p.add_argument('--luna_size', type=str, default='base', choices=['base', 'large', 'huge'])
    # Pretrained weights
    ...
    p.add_argument('--dropout', type=float, default=0.3, help='Dropout probability for head modules')
...
def main():
    ...
    if params.pretrained_weights is None:
        base_dir = os.path.dirname(__file__)
        luna_size = getattr(params, 'luna_size', 'base')
        weights_map = {
        ...
        params.pretrained_weights = weights_map.get(params.model)
    elif not os.path.isabs(params.pretrained_weights):
        params.pretrained_weights = os.path.join(os.path.dirname(__file__), params.pretrained_weights)

    print("=" * 70)
    print(f"MI Fine-Tuning with {params.model.upper()} Backbone")
    print("=" * 70)
    ...
    if use_wandb:
        run_name = params.wandb_run_name or (
            f"MI_{params.model}_{params.dataset}_"
            f"a{params.alpha}_b{params.beta}_{params.expert_feature}_"
            f"{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        tags = [params.dataset.lower(), params.model, 'mi-finetuning', params.expert_feature]
        if params.use_layer_adapters:
            tags.append('layer-adapters')
        wandb.init(
    ...
    # Create backbone
    print(f"\nCreating {params.model} backbone...")
    backbone, backbone_out_dim, token_dim = create_backbone(
        ...
        n_layer_cbramod=params.n_layer_cbramod,
        nhead=params.nhead,
        dim_feedforward=params.dim_feedforward,
        use_layer_adapters=params.use_layer_adapters,
        adapter_reduction=params.adapter_reduction,
        dropout=0.1,
