...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='codebrain',
                        ...
                        help='backbone model: codebrain (SSSM), cbramod (CBraMod), femba (BiMamba), or luna (Cross-Attn)')
...
def main():
    ...
    model_name_map = {
        'codebrain': 'CodeBrain (SSSM)', 'cbramod': 'CBraMod',
        'femba': 'FEMBA (BiMamba)', 'luna': 'LUNA (Cross-Attn)',
    }
    ...
    if params.model == 'cbramod':
        ...
        default_weights = os.path.join(base_dir, f'BioFoundation/checkpoints/LUNA/LUNA_{params.luna_size}.safetensors')
        default_n_layer = 8
    ...
    if params.model == 'cbramod':
    ...
    elif params.model in ('femba', 'luna'):
        # FEMBA/LUNA: MSFT multi-scale not directly applicable to their internal patching.
        # Use backbone_factory + simple head (same as head-only approach).
        from backbone_factory import create_backbone
        luna_size = getattr(params, 'luna_size', 'base') if params.model == 'luna' else 'base'
        ...
        backbone, backbone_out_dim, token_dim = create_backbone(
            ...
            pretrained_weights_path=pretrained_path,
            device=device,
            **{k: v for k, v in preset.items() if k.startswith('luna_')},
        )
        # Build simple classifier on top
        import torch.nn as tnn
        from einops.layers.torch import Rearrange
        output_dim = 1 if dataset_config['task_type'] == 'binary' else num_classes
        model = tnn.Sequential(
            backbone,
            tnn.Flatten(),
            tnn.Linear(backbone_out_dim, 256),
            tnn.ELU(),
            tnn.Dropout(params.dropout),
            tnn.Linear(256, output_dim),
        )
        print(f"  Using {params.model.upper()} backbone with simple head (MSFT multi-scale N/A)")
    else:  # codebrain
