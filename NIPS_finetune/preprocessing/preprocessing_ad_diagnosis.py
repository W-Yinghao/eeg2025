#!/usr/bin/env python3
"""
Unified EEG Dataset Preprocessing Script for AD-related Multi-class Diagnosis

Preprocesses EEG data from multiple sources (EGI raw, EGI preprocessed .mat, and BrainVision formats)
and unifies them to a common channel set and sampling frequency.

Source datasets:
    - eeg_AD+MCI+SCD+HC_EGI_124: EGI preprocessed .mat (EEGLAB), 58 EEG channels, 250 Hz
      -> Already has 58 channels matching the common set; only resample, no channel manipulation
...
Output:
    /projects/EEG-foundation-model/diagnosis_data/ad_diagnosis_preprocessed/eeg_segments.lmdb

Channel unification strategy:
    - AD .mat dataset already has 58 channels after its own preprocessing; kept as-is
    - EGI raw (129 channels) are mapped to 58 common 10-20 channels
    - BP (63 channels) are picked to match the same 58 common 10-20 channels
    - Final output uses 58 channels in 10-20 naming

Preprocessing pipeline:
    For EGI/BP (same as preprocessing_unified_diagnosis.py):
    ...
    For AD .mat files:
        1. Read EEGLAB .mat file (58 channels already present)
        2. Resample to target frequency (200 Hz)
        3. Apply average reference
        4. Segment into fixed-length epochs
        5. Save to LMDB
        (No bandpass/notch filtering - data is already filtered)
        (No channel picking/renaming - 58 channels already match)

Labels (4-class):
    0: AD (Alzheimer's Disease, including MCI, SCD, and HC from eeg_AD+MCI+SCD+HC_EGI_124)
    1: CVD (Cardiovascular Disease)
    2: depression
    3: normal

Usage:
    python preprocessing_ad_diagnosis.py
    python preprocessing_ad_diagnosis.py --target_sfreq 200 --segment_duration 5
    python preprocessing_ad_diagnosis.py --num_workers 8
"""
...
# (Same as preprocessing_unified_diagnosis.py)
...
def extract_subject_id(filename, label_name, system):
    ...
    Naming patterns:
        AD .mat:        data_eeg_all_sub<N>.mat -> subN
    ...
    Args:
        ...
        label_name: 'AD', 'CVD', 'normal', or 'depression'
        system: 'EGI', 'BP', or 'MAT'

    Returns:
        subject_id: Extracted subject identifier
    """
    if system == 'MAT':
        # AD .mat files: data_eeg_all_sub<N>.mat
        match = re.search(r'sub(\d+)', filename)
        if match:
            return f"AD_sub{match.group(1)}"
        return filename.replace('.mat', '')
            ...
            if date_pattern.match(part):
                ...
                if i > 0:
                    ...
                    if candidate not in ['close', 'open', 'before', 'after', 'rest']:
                        prefix = 'normal_BP' if label_name == 'normal' else 'dep_BP'
                        return f"{prefix}_{candidate}"
                    elif i > 1:
                        prefix = 'normal_BP' if label_name == 'normal' else 'dep_BP'
                        return f"{prefix}_{parts[i - 2]}"
                break
...
OUTPUT_DIR = Path("/projects/EEG-foundation-model/diagnosis_data/ad_diagnosis_preprocessed")

DATASETS = {
    'AD_MAT': {
        'folder': DATA_ROOT / "eeg_AD+MCI+SCD+HC_EGI_124",
        'label': 0,
        'label_name': 'AD',
        'system': 'MAT',
        'file_pattern': '.mat',
...
def read_mat_eeglab(file_path):
    """
    Read preprocessed .mat file in EEGLAB format.

    The AD .mat files already have 58 channels after their own preprocessing.
    We read all channels as-is without any channel selection or renaming.
    ...
    ch_names = []
    for ch in chanlocs:
        if len(ch) > 0 and 'labels' in ch.dtype.names:
            ch_names.append(str(ch['labels'][0]))
        else:
            ch_names.append('unknown')
...
# ============================================================================
def preprocess_mat(data, sfreq, ch_names, target_sfreq=TARGET_SFREQ):
    """
    Preprocess already filtered .mat EEG data from the AD dataset.

    The AD .mat files already have 58 channels matching the common channel set.
    We only perform:
        1. Resample to target frequency
        2. Apply average reference

    No channel picking, renaming, bandpass or notch filtering is done.
    """
    n_channels = data.shape[0]
    logger.debug(f"MAT file: {n_channels} channels, sfreq={sfreq}")

    # Resample if needed
    if sfreq != target_sfreq:
        from scipy import signal
        n_samples_new = int(data.shape[1] * target_sfreq / sfreq)
        data = signal.resample(data, n_samples_new, axis=1)

    # Apply average reference
    data = data - np.mean(data, axis=0, keepdims=True)

    return data, target_sfreq, ch_names


def preprocess_egi(raw, target_sfreq=TARGET_SFREQ, low_freq=LOW_FREQ,
                   high_freq=HIGH_FREQ, notch_freq=NOTCH_FREQ):
    """
    Preprocess EGI EEG data (from .mff files).
    Same as preprocessing_unified_diagnosis.py.

    Steps:
        1. Pick only EEG channels
        2. Pick common EGI channels
        3. Rename to 10-20 standard names
...
def segment_data_array(data, sfreq, segment_duration=SEGMENT_DURATION):
    """
    Segment continuous EEG data (numpy array) into fixed-length epochs.
...
def segment_data(raw, segment_duration=SEGMENT_DURATION):
    """
    Segment continuous EEG data from MNE Raw object into fixed-length epochs.
    ...
    return segment_data_array(data, sfreq, segment_duration)
...
def process_single_file(args):
    ...
    try:
        if system == 'MAT':
            # Read and preprocess .mat file
            # All 58 channels kept as-is, only resample + average reference
            data, sfreq, ch_names = read_mat_eeglab(file_path)
            data, sfreq, ch_names = preprocess_mat(data, sfreq, ch_names, target_sfreq)

            # Segment the data
            segments = segment_data_array(data, sfreq, segment_duration)

            result['orig_sfreq'] = 250  # Original sampling rate of .mat files
            result['n_channels'] = len(ch_names)
...
class LMDBWriter:
    ...
    def close(self, target_sfreq, segment_duration, low_freq, high_freq, notch_freq):
        ...
        ad_subjects = [s for s, l in self.subject_label_map.items() if l == 0]
        cvd_subjects = [s for s, l in self.subject_label_map.items() if l == 1]
        depression_subjects = [s for s, l in self.subject_label_map.items() if l == 2]
        normal_subjects = [s for s, l in self.subject_label_map.items() if l == 3]
        ...
        with open(stats_path, 'w') as f:
            f.write("AD-related Diagnosis EEG Preprocessing Statistics\n")
            f.write("=" * 70 + "\n\n")
            ...
            f.write(f"  AD subjects: {len(ad_subjects)}\n")
            ...
            f.write(f"\n  AD subjects: {sorted(ad_subjects)}\n")
            f.write(f"  CVD subjects: {sorted(cvd_subjects)}\n")
        ...
        logger.info(f"Total subjects: {len(self.subjects)} (AD: {len(ad_subjects)}, CVD: {len(cvd_subjects)}, "
...
def main():
    parser = argparse.ArgumentParser(
        description='Unified preprocessing for AD-related multi-source EEG diagnosis data'
    )
    ...
    logger.info("=" * 70)
    logger.info("Unified EEG Preprocessing for AD-related Diagnosis Classification")
    logger.info("(4-class: AD, CVD, depression, normal)")
    logger.info("=" * 70)
