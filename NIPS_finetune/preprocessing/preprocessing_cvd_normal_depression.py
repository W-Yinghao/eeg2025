#!/usr/bin/env python3
"""
CVD vs Normal vs Depression EEG Dataset Preprocessing Script

Preprocesses EEG data from three folders and saves to a single LMDB file for 3-class classification.
...
Source:
    ...
    - /projects/EEG-foundation-model/diagnosis_data/eeg_depression_EGI_21/

Output:
    /projects/EEG-foundation-model/diagnosis_data/cvd_normal_depression_preprocessed/eeg_segments.lmdb

Data format:
    - EGI .mff format (folders)
    - CVD/Normal: 140 channels (129 EEG after pick_types)
    - Depression: 129 channels (129 EEG after pick_types)
    - All have same EEG channel setup: E1-E128 + reference
    - Original sampling rate: 500-1000 Hz

Preprocessing pipeline:
    1. Read EGI .mff files
    2. Pick EEG channels only (exclude stim channels)
...
Usage:
    python preprocessing_cvd_normal_depression.py
    python preprocessing_cvd_normal_depression.py --target_sfreq 200 --segment_duration 5
    python preprocessing_cvd_normal_depression.py --num_workers 8
"""
...
def extract_subject_id(foldername, label_name):
    ...
    if label_name == 'normal':
        ...
        # Subject is before the first hyphen
        if '-' in name:
            return name.split('-')[0]
        elif len(parts) >= 1:
            return parts[0]
...
OUTPUT_DIR = Path("/projects/EEG-foundation-model/diagnosis_data/cvd_normal_depression_preprocessed")
...
# ============================================================================
class LMDBWriter:
    """LMDB writer for 3-class data with streaming support."""
    ...
    def close(self, target_sfreq, segment_duration, low_freq, high_freq, notch_freq):
        ...
        with open(stats_path, 'w') as f:
            f.write("CVD vs Normal vs Depression EEG Preprocessing Statistics\n")
            f.write("=" * 60 + "\n\n")
        ...
        logger.info(f"Total subjects: {len(self.subjects)} (CVD: {len(cvd_subjects)}, normal: {len(normal_subjects)}, depression: {len(depression_subjects)})")
...
def main():
    parser = argparse.ArgumentParser(
        description='Preprocess CVD vs Normal vs Depression EEG data for 3-class classification'
    )
    ...
    logger.info("=" * 60)
    logger.info("CVD vs Normal vs Depression EEG Preprocessing")
    logger.info("(3-class classification)")
    logger.info("=" * 60)
