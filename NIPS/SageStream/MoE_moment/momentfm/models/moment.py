    ...
    def _get_transformer_backbone(self, config) -> nn.Module:
        ...
        if config.getattr("enable_gradient_checkpointing", True):
            try:
                ...
                # Older transformers: enable checkpointing then patch to use_reentrant=False
                # to avoid broken backward pass and PyTorch >= 2.9 warnings
                import functools
                import torch.utils.checkpoint as torch_ckpt
                transformer_backbone.gradient_checkpointing_enable()
                _orig_checkpoint = torch_ckpt.checkpoint
                ...
                torch_ckpt.checkpoint = _patched_checkpoint
