...
class PatchEmbed(nn.Module):
    def __init__(self, EEG_size=6000, patch_size=200, in_chans=1, embed_dim=200):
        super().__init__()
...
class Conv(nn.Module):
    def __init__(self, in_chans=1, out_chans=8, dropout=0.3, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.conv = nn.Sequential(
            nn.Conv2d(in_chans, out_chans, kernel_size=(1, 15), stride=(1, 8), padding=(0, 7)),
            nn.BatchNorm2d(out_chans),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Conv2d(out_chans, out_chans // 2, kernel_size=(1, 3), padding=(0, 1)),
            nn.BatchNorm2d(out_chans // 2),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Conv2d(out_chans // 2, out_chans // 2, kernel_size=(1, 3), padding=(0, 1)),
            nn.BatchNorm2d(out_chans // 2),
            nn.ReLU(),
            nn.Dropout(dropout)
        )
...
class TFConv(nn.Module):
    def __init__(self, in_chans=1, out_chans=8, dropout=0.3):
        super().__init__()
        self.t_convs = Conv(in_chans, out_chans, dropout)
        self.f_convs = Conv(in_chans, out_chans, dropout)

    def forward(self, x, **kwargs):
        x_fft = torch.fft.fft(x, dim=-1).real
        x = rearrange(x, 'B N A T -> B (N A) T')
        x_fft = rearrange(x_fft, 'B N A T -> B (N A) T')
        B, NA, T = x.shape
        x = x.unsqueeze(1)
        x_fft = x_fft.unsqueeze(1)
        x = self.t_convs(x)
        x_fft = self.f_convs(x_fft)
        x = rearrange(x, 'B C NA T -> B NA (T C)')
        x_fft = rearrange(x_fft, 'B C NA T -> B NA (T C)')
        x = torch.cat((x, x_fft), dim=-1)
        return x


class NeuralTransformer(nn.Module):
    def __init__(self, EEG_size=6000, patch_size=200, in_chans=1, out_chans=8,
                 num_classes=1000, embed_dim=200, depth=12, num_heads=10, mlp_ratio=4.,
                 qkv_bias=False, qk_norm=None, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
        ...
        self.patch_embed = TFConv(out_chans=out_chans) if in_chans == 1 \
            else PatchEmbed(EEG_size=EEG_size, patch_size=patch_size, in_chans=in_chans,
                            embed_dim=embed_dim)
    ...
    def forward_features(self, x, input_chans=list(range(20)), return_patch_tokens=False,
        ...
        if self.time_embed is not None:
            ...
            time_embed = self.time_embed[:, 0: input_time_window, :].unsqueeze(1).expand(batch_size, nc, -1,
                                                                                         -1).flatten(1, 2)
            x[:, 1:, :] += time_embed
    ...
    def forward(self, x, input_chans=list(range(20)), return_patch_tokens=False, return_all_tokens=False, **kwargs):
